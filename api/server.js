...
app.get("/api/energy/usage-hours", authenticateToken, async (req, res) => {
  try {
    const [usage] = await pool.query(
      `SELECT DATE(timestamp) as date, HOUR(timestamp) as hour, SUM(energy_usage) as total_usage 
       FROM energy_data 
       JOIN devices ON energy_data.device_id = devices.id 
       WHERE devices.user_id = ? AND DATE(timestamp) = CURDATE()
       GROUP BY DATE(timestamp), HOUR(timestamp) 
       ORDER BY DATE(timestamp) DESC, HOUR(timestamp) ASC`,
      [req.user.id]
    );
    res.json(usage);
  } catch (error) {
    res.status(500).json({ error: "Error fetching hourly energy usage" });
  }
});
...
setInterval(async () => {
  try {
    ...
    for (const device of devices) {
      const usage = (Math.random() * 5).toFixed(2); // Random usage between 0 and 5 kWh
      await pool.query("INSERT INTO energy_data (device_id, energy_usage, timestamp) VALUES (?, ?, NOW())", [
        device.id,
        usage,
      ]);
    }
